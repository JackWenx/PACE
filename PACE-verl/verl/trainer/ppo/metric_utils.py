...
def process_validation_metrics(
    ...
    """
    data_src2var2idx = defaultdict(lambda: defaultdict(int))
    # Group metrics by data source, prompt and variable
    data_src2prompt2var2vals = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
    for sample_idx, data_source in enumerate(data_sources):
        prompt = sample_inputs[sample_idx]
        var2vals = data_src2prompt2var2vals[data_source][prompt]
        var2idx = data_src2var2idx[data_source]
        for var_name, var_vals in infos_dict.items():
            try:
                var_idx = var2idx[var_name]
                var2vals[var_name].append(var_vals[var_idx])
                var2idx[var_name] = var_idx + 1
                # print(f'var idx: {var_idx}')
                # print(f'len var_vals: {len(var_vals)}')
            except Exception as e:
                print(f'var_name: {var_name}, sample_idx: {sample_idx}, len var_vals: {len(var_vals)}')




    # print(f'data_src2idx: {data_src2idx}')
    # Calculate metrics for each group
    data_src2prompt2var2metric = defaultdict(lambda: defaultdict(lambda: defaultdict(dict)))
    for data_source, prompt2var2vals in data_src2prompt2var2vals.items():
        for prompt, var2vals in prompt2var2vals.items():
            for var_name, var_vals in var2vals.items():
                if isinstance(var_vals[0], (str, dict)):
                    continue
