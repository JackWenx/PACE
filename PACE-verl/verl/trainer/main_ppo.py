...
from verl.workers.reward_manager import naive
...
def run_ppo(config) -> None:
    ...
    if not ray.is_initialized():
        ...
        print(f'Initializing tensorboear at {TENSORBOARD_DIR}')
...
class TaskRunner:
    def run(self, config):
        ...
        # 测试的时候先用默认的reward_manager
        val_reward_fn = load_reward_manager(config, tokenizer, num_examine=1, **config.reward_model.get("reward_kwargs", {}))
        # val_reward_fn = DAPORewardManager(
        #     tokenizer=tokenizer,
        #     num_examine=1,
        ...
        # )
        print(f"Using reward function for validation: {val_reward_fn.__class__.__name__}")
